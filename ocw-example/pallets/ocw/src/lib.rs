...
pub mod pallet {
    ...
    // 备注：Price结构目前仅有priceUsd，以后可以扩展更多的信息内容。
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn fetch_price_info(block_number: T::BlockNumber) -> Result<(), Error<T>> {
            ...
            let price = Self::fetch().map_err(|_| <Error<T>>::HttpFetchingError)?;

            // 利用 PriceEnity 结构，在 ocw 的 local storage 存入价格信息的Vec格式。
            let price_info: PriceEnity = PriceEnity {
            ...
            let parsed_price = match Self::parse(&price_str) {
        ...
        fn fetch() -> Result<Vec<u8>, Error<T>> {
        ...
        fn parse(price_str: &str) -> Option<(u64, Permill)> {
